...
\usepackage{url}
\usepackage{booktabs}
\title{Latent Fixture Inference: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about tool use and manipulation. The original research bet was: Infer unseen fixtures and environmental supports from action resistance patterns. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Grasp to Act: Dexterous Grasping for Tool Use in Dynamic Settings (2026)
\item PI-VLA: Adaptive Symmetry-Aware Decision-Making for Long-Horizon Vision-Language-Action Manipulation (2026)
\item A State-Adaptive Koopman Control Framework for Real-Time Deformable Tool Manipulation in Robotic Environmental Swabbing (2026)
\item PLATO: Planning with LLMs and Affordances for Tool Manipulation (2026)
\item LangForce: Bayesian Decomposition of Vision Language Action Models via Latent Action Queries (2026)
\end{itemize}
